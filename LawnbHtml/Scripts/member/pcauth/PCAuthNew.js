    ...
    try {
        ...
        $.ajax({
            ...
            success: function (result) {
                CheckKey = result.trim();

                var params = { "AuthType": "New", "UserID": UserID, "UserPW": UserPW, "AuthName": AuthName, "CheckKey": CheckKey }
                $("#bg_screen").show();
                sendNonBlockingAjaxJsonRequest("/PCAuth/PCAuthSet/", params, function (result) {
                    if (result[0]["ResultCode"] == "0") {
                        localStorage.setItem("LawnbPCAuth_" + UserID, CheckKey);
                        //var HttpsUrl = "https://www.lawnb.com/Member/LocalStorage/?UserID=" + encodeURI(UserID) + "&PCAuth=" + encodeURI(CheckKey);
                        //var HttpUrl = "http://www.lawnb.com/Member/LocalStorage/?UserID=" + encodeURI(UserID) + "&PCAuth=" + encodeURI(CheckKey);

                        //$("#banner_list_pcauth").append("<iframe height=\"1px\" style=\"display:none;\" src=\"" + HttpsUrl + "\"></iframe>");
                        //$("#banner_list_pcauth").append("<iframe height=\"1px\" style=\"display:none;\" src=\"" + HttpUrl + "\"></iframe>");
            ...
            error: function (e) {
                //console.log(e);
                var params = { "AuthType": "New", "UserID": UserID, "UserPW": UserPW, "AuthName": AuthName }
                $("#bg_screen").show();
