<?php include '../include/header_sub_content.php'; ?>
<!--	시작 : 헤더	-->
<header class="ct-header">
	<div class="w1600 str clear">
		<h1 class="logo-div"><a href="../main/main_search.php"><img src="../images/common/logo_s.png" alt="로앤비"/></a>
		</h1>
	</div>
</header>
<!--	끝 : 헤더	-->
<div class="container-wrap">
	<div class="container-inner">
		<div class="container-left">
			<div class="scroll-y">
				<div class="container-left-pd">
					<div class="index-list">
						<ul class="index-ul">
							<li><a href="user_manage_list01.php">ID 관리</a></li>
							<li><a href="user_manage_list02.php">유로권한 결제</a></li>
							<li class="on"><a href="user_manage_list03.php">ID 결제 관리</a></li>
							<li><a href="user_manage_list04.php">결제내역 조회</a></li>
						</ul>
					</div>
					<div class="mt-20">
						<a href="user_manage_list05.php" class="btn-none btn-m dis-b ta-c">1:1 문의</a>
						<a href="user_manage_list05.php" class="btn-none btn-m dis-b ta-c mt-10">탈퇴 신청</a>
					</div>
				</div>
			</div>
		</div>
		<div class="container-cr manage-id-container">
			<h2 class="sub-main-tit">
				<p>ID 결제 관리</p>
			</h2>
			<div class="bg-border p-25">
				<div class="content-div-wrap">
					<div class="h3-wrap">
						<div class="basic_info_box bg-sky-box mt-15 lh-26px">
							<p><b>이세진 lsj1027 ( 개인회원 )</b></p>
							<p>유료회원 (Professional), 현등급 이용기간 : 2011-04-20 ~ 2100-12-31</p>
						</div>
						<div class="info_table">
							<table class="mt-20">
								<tbody>
								<tr>
									<td>
										<ul>
											<li>그룹ID 총 5개 (Basic 1개, Professional 0개, 무료 4개)
												<button class="btn btn-blue btn-s ml-5">목록</button>
											</li>
											<li>
												<div>다음 결제 예정일은 <b>2024년 01월 01일</b>입니다.</div>
												<div class="mt-5">
													<button class="btn btn-square square-none btn-s mr-5">정기결제 해지
													</button>
													<p class="dis-ib str pl-20 c-58 lh-20px">
														<span class="n-i-w15 n-i-alert-cir n-i str-ltc"></span>
														정기결제 기능만 해제되며, 현재 서비스 만료일까지 해당 등급으로 서비스를 계속 이용하실 수 있습니다.
													</p>
												</div>
											</li>
										</ul>
									</td>
								</tr>
								</tbody>
							</table>
						</div>
					</div>
					<div class="h3-wrap">
						<div class="h3-div clear">
							<div class="c-left">
								<h3>로앤비 정기결제 정보</h3>
							</div>
						</div>
						<div class="info_table">
							<table>
								<colgroup>
									<col width="100px">
									<col>
								</colgroup>
								<tbody>
								<tr>
									<th>로앤비 법률정보</th>
									<td>
										<p>현재 권한 : 유로회원(Basic 2023.12.01 ~ 2024.01.01)</p>
										<ul class="mt-15">
											<li>다음 결제 예정일 : Basic, 2024.01.01</li>
											<li>결제 예정금액 : 103,000원</li>
											<li>정기결제 : 1개월</li>
											<li>결제수단 : 신용카드(신한 / 45184212****302*)
												<button class="btn square-none btn-square btn-s ml-5">변경</button>
											</li>
										</ul>
									</td>
								</tr>
								</tbody>
							</table>
						</div>
					</div>
					<div class="h3-wrap">
						<div class="h3-div clear">
							<div class="c-left">
								<h3>금액 안내</h3>
							</div>
						</div>
						<div class="info_table">
							<table class="list">
								<thead>
								<tr>
									<th>등급</th>
									<th>1개월</th>
									<th class="c-red fw-700">12개월 (20% 할인 혜택!)</th>
								</tr>
								</thead>
								<tbody>
									<tr>
										<td>Basic(베이직)</td>
										<td class="ta-c">113,300원</td>
										<td class="ta-c"><strike>1,359,600원</strike>  → 1,087,680원</td>
									</tr>
									<tr>
										<td>Professional(프로페셔널)</td>
										<td class="ta-c">168,300원​</td>
										<td class="ta-c"><strike>1,359,600원</strike>  → 1,087,680원</td>
									</tr>
								</tbody>
							</table>
							<ul class="text_ex">
								<li>Professional 등급 결제시 국내 유일의 온라인 특별법 주석서 “온주”를 이용하실 수 있습니다.</li>
								<li>가격 변동 시 정기결제 금액이 조정될 수 있습니다.</li>
							</ul>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<?php include '../include/footer_sub.php'; ?>
